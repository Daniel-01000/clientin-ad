...
function AnimFounderPreLaunch({ template }: { template: AdTemplate }) {
  ...
  const [idx, setIdx] = useState(0);
  useEffect(() => {
    const t = setTimeout(() => setIdx(i => (i + 1) % lines.length), 4000);
    return () => clearTimeout(t);
  }, [idx, lines.length]);
...
export default function AdCard({ template }: Props) {
  ...
  const downloadPortrait = async () => {
    ...
    const w = 1080;
    const h = 1350;
    // Render the square at 1080 then draw onto a taller canvas with padding
    const squareUrl = await toPng(adRef.current, {
      width: w, height: w, canvasWidth: w, canvasHeight: w,
      pixelRatio: 1, cacheBust: true, backgroundColor: '#0a0a0a',
      style: { transform: 'none', borderRadius: '0' },
    });
