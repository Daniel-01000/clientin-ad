...
export default function ScreenshotUploader({ screenshots, onChange }: Props) {
  ...
  // Keep a ref to always have the latest screenshots in async callbacks
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      const merged = [...screenshotsRef.current, ...newScreenshots];
      onChange(merged);
      screenshotsRef.current = merged;
      ...
      for (const screenshot of newScreenshots) {
        ...
        try {
          ...
          // Use ref so we always patch the latest list, not a stale closure
          const updated = screenshotsRef.current.map((s) =>
